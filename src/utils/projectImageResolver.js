...
function isNlpProject(project = {}) {
  ...
  return (
    source.includes("nlp") ||
    source.includes("bencana") ||
    source.includes("xlm-roberta")
  );
}
...
function isClusteringProject(project = {}) {
  const source = getProjectSourceText(project);
  return (
    source.includes("clustering") ||
    source.includes("k-means") ||
    source.includes("nba")
  );
}
...
function getLocalPhotoFallback(project = {}) {
  ...
  if (
    source.includes("tmovie") ||
    source.includes("web movie") ||
    source.includes("movie catalog")
  ) {
    return "/project-photos/web-movie.svg";
  }

  if (
    source.includes("clothing store") ||
    source.includes("web shopping") ||
  ...
  if (
    ...
    source.includes("android hotel")
  ) {
    return "/project-photos/react-native-hotel.svg";
  }

  if (
    source.includes("covid news") ||
    source.includes("web berita") ||
    source.includes("news portal")
  ) {
    return "/project-photos/covid-news-react-api.svg";
  }

  if (
    source.includes("web portfolio pribadi") ||
    source.includes("portfolio pribadi") ||
    source.includes("portfolio react")
  ) {
